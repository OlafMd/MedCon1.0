/* 
 * Generated on 07/24/15 15:26:20
...
namespace MMDocConnectDBMethods.Case.Atomic.Manipulation
{
    ...
    /// var result = cls_Drop_Treatment_Count_Temporary_Table.Invoke(connectionString).Result;
    /// </example>
    /// </summary>
	[DataContract]
	public class cls_Drop_Treatment_Count_Temporary_Table
	{
		...
		protected static FR_Base Execute(DbConnection Connection,DbTransaction Transaction,CSV2Core.SessionSecurity.SessionSecurityTicket securityTicket = null){
			...
			var commandLocation = "MMDocConnectDBMethods.Case.Atomic.Manipulation.SQL.cls_Drop_Treatment_Count_Temporary_Table.sql";
			...
			{
				throw new Exception("Exception occured while executing non query cls_Drop_Treatment_Count_Temporary_Table",ex);
			}
		...
		protected static FR_Base Invoke(DbConnection Connection, DbTransaction Transaction, string ConnectionString,CSV2Core.SessionSecurity.SessionSecurityTicket securityTicket = null)
		{
			...
			catch (Exception ex)
			{
				...
				throw new Exception("Exception occured in method cls_Drop_Treatment_Count_Temporary_Table",ex);
			}
...
[OperationContract]
FR_Base cls_Drop_Treatment_Count_Temporary_Table(string sessionToken = null)
{
	try
	{
		var securityTicket = Verify(sessionToken);
		FR_Base invocationResult = cls_Drop_Treatment_Count_Temporary_Table.Invoke(connectionString,securityTicket);
