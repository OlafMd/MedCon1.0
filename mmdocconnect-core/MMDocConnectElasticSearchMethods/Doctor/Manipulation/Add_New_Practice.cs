...
{
    public static class Add_New_Practice
    {
        ...
        public static void Import_Practice_Data_to_ElasticDB(List<Practice_Doctors_Model> DPModelL, string indexName)
        {
            ...
            if (!type_exists)
            {
                ...
                string resultProductMapping = connection.Put(new PutMappingCommand(indexName, "user"), jsonProductMapping);
            }

            Elastic_Utils.BulkType<Practice_Doctors_Model>(DPModelL, connection, serializer, indexName, "user");

        }


        public static void Delete_Doctor_Practice(string index_name, string id)
        {
            var command = Commands.Delete(index_name, "user", id);
            var connection = Elastic_Utils.ElsaticConnection();
