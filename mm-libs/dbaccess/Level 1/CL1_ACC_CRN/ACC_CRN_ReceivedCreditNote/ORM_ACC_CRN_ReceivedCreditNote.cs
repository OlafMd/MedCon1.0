...
{
	[Serializable]
	public class ORM_ACC_CRN_ReceivedCreditNote
	{
		public static readonly string TableName = "acc_crn_receivedcreditnotes";
		public bool Status_IsAlreadySaved { get; set; }
		public bool Status_IsDirty { get; set; }
		public static readonly int QueryTimeout = 60;

		#region Class Constructor
		public ORM_ACC_CRN_ReceivedCreditNote()
		{
			Status_IsAlreadySaved = false;
			Status_IsDirty = false;
			//When creating a new class, generate a new UUID
			_ACC_CRN_ReceivedCreditNoteID = Guid.NewGuid();
			_IsDeleted = false;
		...
		private Guid _ACC_CRN_ReceivedCreditNoteID = default(Guid);
		...
		public Guid ACC_CRN_ReceivedCreditNoteID { 
			get {
				return _ACC_CRN_ReceivedCreditNoteID;
			}
			set {
				if(_ACC_CRN_ReceivedCreditNoteID != value){
					_ACC_CRN_ReceivedCreditNoteID = value;
					Status_IsDirty = true;
				}
			}
		} 
		...
		protected FR_Base Save(DbConnection Connection, DbTransaction Transaction, string ConnectionString)
		{
			...
			try
			{
				...
				if (saveORMClass == true) { 
					...
					{
						Query = new System.IO.StreamReader(System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("CL1_ACC_CRN.ACC_CRN_ReceivedCreditNote.SQL.Update.sql")).ReadToEnd();
					}
					else
					{
						Query = new System.IO.StreamReader(System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("CL1_ACC_CRN.ACC_CRN_ReceivedCreditNote.SQL.Insert.sql")).ReadToEnd();
					}
					...
					CSV2Core_MySQL.Support.DBSQLSupport.SetParameter(command, "ACC_CRN_ReceivedCreditNoteID", _ACC_CRN_ReceivedCreditNoteID);
		...
		private FR_Base Load(DbConnection Connection, DbTransaction Transaction, Guid ObjectID, string ConnectionString)
		{
			...
			try
			{
				...
				string SelectQuery = new System.IO.StreamReader(System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("CL1_ACC_CRN.ACC_CRN_ReceivedCreditNote.SQL.Select.sql")).ReadToEnd();
				...
				_ACC_CRN_ReceivedCreditNoteID = Guid.Empty;
				CSV2Core_MySQL.Support.DBSQLSupport.SetParameter(command,"ACC_CRN_ReceivedCreditNoteID", ObjectID );
				...
				try
				{
					...
					reader.SetOrdinals(new string[] { "ACC_CRN_ReceivedCreditNoteID","CreditNoteITPL","CreditNote_Currency_RefID","CreditNote_Value","CreditNote_Number","DateOnCreditNote","Creation_Timestamp","Tenant_RefID","IsDeleted" });
					if (reader.HasRows == true)
					{
						reader.Read(); //Single result only
						//0:Parameter ACC_CRN_ReceivedCreditNoteID of type Guid
						_ACC_CRN_ReceivedCreditNoteID = reader.GetGuid(0);
					...
					if(_ACC_CRN_ReceivedCreditNoteID != Guid.Empty){
						//Successfully loaded
						Status_IsAlreadySaved = true;
						Status_IsDirty = false;
					} else {
        ...
        {
			public Guid? ACC_CRN_ReceivedCreditNoteID {	get; set; }
			...
			#region Search
			public static List<ORM_ACC_CRN_ReceivedCreditNote> Search(string connectionString, Query query)
			{
			    return Query.Search(query, connectionString, null, null);
			}

			public static List<ORM_ACC_CRN_ReceivedCreditNote> Search(DbConnection connection, Query query)
			{
			    return Query.Search(query, null, connection, null);
			}

			public static List<ORM_ACC_CRN_ReceivedCreditNote> Search(DbConnection connection, DbTransaction transaction, Query query)
			{
			    return Query.Search(query, null, connection, transaction);
			}

			private static List<ORM_ACC_CRN_ReceivedCreditNote> Search(Query query, string connectionString, DbConnection connection, DbTransaction transaction)
			{
				CSV2Core.Core.Interfaces.IManagedConnection managedConnection = new CSV2Core_MySQL.MySQLManagedConnection();
			    List<ORM_ACC_CRN_ReceivedCreditNote> items;
				...
				try
			    {
			        ...
			        items = new List<ORM_ACC_CRN_ReceivedCreditNote>();

			        var reader = new CSV2Core_MySQL.Support.DBSQLReader(command.ExecuteReader());
					reader.SetOrdinals(new string[] { "ACC_CRN_ReceivedCreditNoteID","CreditNoteITPL","CreditNote_Currency_RefID","CreditNote_Value","CreditNote_Number","DateOnCreditNote","Creation_Timestamp","Tenant_RefID","IsDeleted" });
			        while (reader.Read())
			        {
			            ORM_ACC_CRN_ReceivedCreditNote item = new ORM_ACC_CRN_ReceivedCreditNote();
						//0:Parameter ACC_CRN_ReceivedCreditNoteID of type Guid
						item.ACC_CRN_ReceivedCreditNoteID = reader.GetGuid(0);
						//1:Parameter CreditNoteITPL of type String
