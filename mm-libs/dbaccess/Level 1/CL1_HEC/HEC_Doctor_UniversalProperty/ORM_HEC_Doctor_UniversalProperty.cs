/* 
 * Generated on 09/01/15 14:39:43
...
{
	[Serializable]
	public class ORM_HEC_Doctor_UniversalProperty
	{
		public static readonly string TableName = "hec_doctor_universalproperties";
		public bool Status_IsAlreadySaved { get; set; }
		public bool Status_IsDirty { get; set; }
		public static readonly int QueryTimeout = 60;

		#region Class Constructor
		public ORM_HEC_Doctor_UniversalProperty()
		{
			Status_IsAlreadySaved = false;
			Status_IsDirty = false;
			//When creating a new class, generate a new UUID
			_HEC_Doctor_UniversalPropertyID = Guid.NewGuid();
			_IsDeleted = false;
		...
		private Guid _HEC_Doctor_UniversalPropertyID = default(Guid);
		...
		public Guid HEC_Doctor_UniversalPropertyID { 
			get {
				return _HEC_Doctor_UniversalPropertyID;
			}
			set {
				if(_HEC_Doctor_UniversalPropertyID != value){
					_HEC_Doctor_UniversalPropertyID = value;
					Status_IsDirty = true;
				}
			}
		} 
		...
		protected FR_Base Save(DbConnection Connection, DbTransaction Transaction, string ConnectionString)
		{
			...
			try
			{
				...
				if (saveORMClass == true) { 
					...
					{
						Query = new System.IO.StreamReader(System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("CL1_HEC.HEC_Doctor_UniversalProperty.SQL.Update.sql")).ReadToEnd();
					}
					else
					{
						Query = new System.IO.StreamReader(System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("CL1_HEC.HEC_Doctor_UniversalProperty.SQL.Insert.sql")).ReadToEnd();
					}
					...
					CSV2Core_MySQL.Support.DBSQLSupport.SetParameter(command, "HEC_Doctor_UniversalPropertyID", _HEC_Doctor_UniversalPropertyID);
		...
		private FR_Base Load(DbConnection Connection, DbTransaction Transaction, Guid ObjectID, string ConnectionString)
		{
			...
			try
			{
				...
				string SelectQuery = new System.IO.StreamReader(System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("CL1_HEC.HEC_Doctor_UniversalProperty.SQL.Select.sql")).ReadToEnd();
				...
				_HEC_Doctor_UniversalPropertyID = Guid.Empty;
				CSV2Core_MySQL.Support.DBSQLSupport.SetParameter(command,"HEC_Doctor_UniversalPropertyID", ObjectID );
				...
				try
				{
					...
					reader.SetOrdinals(new string[] { "HEC_Doctor_UniversalPropertyID","GlobalPropertyMatchingID","PropertyName","IsValue_String","IsValue_Number","IsValue_Boolean","Creation_Timestamp","Tenant_RefID","IsDeleted","Modification_Timestamp" });
					if (reader.HasRows == true)
					{
						reader.Read(); //Single result only
						//0:Parameter HEC_Doctor_UniversalPropertyID of type Guid
						_HEC_Doctor_UniversalPropertyID = reader.GetGuid(0);
					...
					if(_HEC_Doctor_UniversalPropertyID != Guid.Empty){
						//Successfully loaded
						Status_IsAlreadySaved = true;
						Status_IsDirty = false;
					} else {
        ...
        {
			public Guid? HEC_Doctor_UniversalPropertyID {	get; set; }
			...
			#region Search
			public static List<ORM_HEC_Doctor_UniversalProperty> Search(string connectionString, Query query)
			{
			    return Query.Search(query, connectionString, null, null);
			}

			public static List<ORM_HEC_Doctor_UniversalProperty> Search(DbConnection connection, Query query)
			{
			    return Query.Search(query, null, connection, null);
			}

			public static List<ORM_HEC_Doctor_UniversalProperty> Search(DbConnection connection, DbTransaction transaction, Query query)
			{
			    return Query.Search(query, null, connection, transaction);
			}

			private static List<ORM_HEC_Doctor_UniversalProperty> Search(Query query, string connectionString, DbConnection connection, DbTransaction transaction)
			{
				CSV2Core.Core.Interfaces.IManagedConnection managedConnection = new CSV2Core_MySQL.MySQLManagedConnection();
			    List<ORM_HEC_Doctor_UniversalProperty> items;
				...
				try
			    {
			        ...
			        items = new List<ORM_HEC_Doctor_UniversalProperty>();

			        var reader = new CSV2Core_MySQL.Support.DBSQLReader(command.ExecuteReader());
					reader.SetOrdinals(new string[] { "HEC_Doctor_UniversalPropertyID","GlobalPropertyMatchingID","PropertyName","IsValue_String","IsValue_Number","IsValue_Boolean","Creation_Timestamp","Tenant_RefID","IsDeleted","Modification_Timestamp" });
			        while (reader.Read())
			        {
			            ORM_HEC_Doctor_UniversalProperty item = new ORM_HEC_Doctor_UniversalProperty();
						//0:Parameter HEC_Doctor_UniversalPropertyID of type Guid
						item.HEC_Doctor_UniversalPropertyID = reader.GetGuid(0);
						//1:Parameter GlobalPropertyMatchingID of type String
