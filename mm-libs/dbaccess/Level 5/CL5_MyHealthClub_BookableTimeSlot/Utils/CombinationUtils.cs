...
namespace CL5_MyHealtClub_Slot.Utils
{
    public class CombinationUtils
    {
        ...
        {
            var retVal = calculatedCombination.StaffCombination.Data.Count == persistedCombination.Staff.Length;

            if (retVal)
            {
                if (calculatedCombination.IsDeviceNeeded)
                {
                    retVal = calculatedCombination.DeviceInstancesCombination.Data.Count == persistedCombination.Devices.Length;
                }
                else
                {
                    retVal = persistedCombination.Devices.Length == 0;
                }
            }
