...
namespace CL5_MyHealtClub_Slot.Utils
{
    ...
    {
        public static Practice ConvertPracticeDBData(Guid officeID, L5TE_GNWTfOID_1506[] officeNonWorkTimes, L5TE_GSHfOID_1540[] officeStandardHours, L5ATW_ANfTID_1855[] appointmentTypesDB)
        {
        ...
        public static List<Staff> ConvertStaffDBData(Guid officeID, L5TE_GSAfT_1645[] doctorsDB, ORM_HEC_Doctor[] hecDoctorsDB, ORM_HEC_Doctor_AssignableAppointmentType[] doc2AATypes, L5TE_GTEFAS_1440[] doctorsExceptions, L3P_GPfT_1537[] professionsForTenant)
        {
            ...
            foreach (var doc in doctorsDB)
            {
                ...
                retVal.Add(new Staff()
                {
                    ...
                    Professions = ConvertDoctorProfessions(doc.Professions, professionsForTenant),
                    Skills = ConvertDoctorSkills(doc.Skills.Select(s => s.SkillID).ToList()),
                    WebAvailabilities = ConvertDoctorAvailabilities(doc.Availability, officeID, true),
                    Availabilities = ConvertDoctorAvailabilities(doc.Availability, officeID, false),
                    ...
                    IsAvailabeForWebBooking = doc.IsWebExcluded == null
                });
            }
            return retVal;
        }
        ...
        private static List<Availability> ConvertDoctorAvailabilities(L5TE_GSAfT_1645_Availability[] doctorAvaDB, Guid officeID, bool webBookable)
        {
            ...
            if (webBookable)
                ...
                doctorAvaDBforOffice = doctorAvaDBforOffice.Where(w => w.GlobalPropertyMatchingID == EnumUtils.GetEnumDescription(AvailabilityType.Standard)).ToArray();
        ...
        private static List<StaffProfession> ConvertDoctorProfessions(L5TE_GSAfT_1645_Profession[] doctorsProfessions, L3P_GPfT_1537[] allProfessions)
        {
            List<StaffProfession> retVal = new List<StaffProfession>();

            if (doctorsProfessions != null && allProfessions != null)
            {
                foreach (var profession in doctorsProfessions)
                {
                    retVal.Add(new StaffProfession()
                    {
                        ID = profession.Profession_RefID,
                        IsDefault = profession.IsPrimary,
                        Skills = ConvertDoctorSkillsInProfessions(allProfessions.FirstOrDefault(f => f.CMN_STR_ProfessionID == profession.Profession_RefID))
                    });
                }
            }
        ...
        private static List<Skill> ConvertDoctorSkillsInProfessions(L3P_GPfT_1537 profession)
        {
        ...
        private static List<AppointmentType> ConvertAppointmentTypes(L5ATW_ANfTID_1855[] appointmentTypesDB)
        {
            ...
            foreach (var type in appointmentTypesDB)
            {
                ...
                retVal.Add(new AppointmentType()
                {
                    ...
                    IsWebBookable = type.RegularType == null
                });
            }
        ...
        private static List<ReqStaffByAbillity> ConvertReqStaffByAbillity(L5ATW_ANfTID_1855_staff[] reqs)
        {
        ...
        private static List<RequiredDeviceType> ConvertReqDeviceTypes(L5ATW_ANfTID_1855_Device[] deviceTypes)
        {
