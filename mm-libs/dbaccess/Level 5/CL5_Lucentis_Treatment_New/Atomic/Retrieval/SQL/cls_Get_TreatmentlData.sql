...
Select
  Concat_Ws(' ', patient_personInfo.FirstName, patient_personInfo.LastName) As
  patient_name,
  Case
  ...
  End As date,
  healthInsurance_bussinessParticipant.DisplayName As health_insurance,
  Case
    When (treatment.IsTreatmentPerformed = 0 And treatment.IsTreatmentBilled = 0
    And treatment.IsScheduled = 1) Then 'sheduled'
    When (treatment.IsTreatmentPerformed = 1 And treatment.IsTreatmentBilled =
    0) Then 'performed' When (treatment.IsTreatmentBilled = 1) Then 'billed'
  End As status,
  Case
    When (treatment.IsTreatmentPerformed = 1) Then Concat_Ws(' ',
    scheduledDoctor_personinfo.Title, scheduledDoctor_personinfo.FirstName,
    scheduledDoctor_personinfo.LastName)
    When (treatment.IsTreatmentPerformed = 0 And treatment.IsScheduled =
    1) Then Concat_Ws(' ', performed_doctor_personinfo.Title,
    performed_doctor_personinfo.FirstName, performed_doctor_personinfo.LastName)
  End As doctor,
  medical_practice.DisplayName As practice,
  treatment.HEC_Patient_TreatmentID As id
From
