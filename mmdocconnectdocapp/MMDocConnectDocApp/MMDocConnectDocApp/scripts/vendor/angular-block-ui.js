...
(function (angular) {
    ...
    blkUI.directive('blockUi', ["blockUiCompileFn", function (blockUiCompileFn) {
        ...
        return function ($scope, $element, $attrs) {
            ...
            $scope.$watch('$_blockUiState.blockCount > 0', function (value) {
                ...
                if (value) {
                    $element.on('DOMMouseScroll', prevent);
                    $element.on('mousewheel', prevent);
                    $element.on('keydown', prevent);
                } else {
                    $element.off('DOMMouseScroll', prevent);
                    $element.off('mousewheel', prevent);
                    $element.off('keydown', prevent);
                };
            });
    ...
    angular.module('blockUI').run(['$templateCache', function ($templateCache) {
        $templateCache.put('angular-block-ui/angular-block-ui.ng.html', '<div class=\"block-ui-overlay\"><div class="loader"></div></div>');
