...
namespace DataImporter.DBMethods.Doctor.Atomic.Retrieval
{
    ...
    /// var result = cls_Check_Login_Email.Invoke(connectionString).Result;
    /// </example>
    /// </summary>
	[DataContract]
	public class cls_Check_Login_Email
	{
		...
		protected static FR_DO_CLE_1414 Execute(DbConnection Connection,DbTransaction Transaction,P_DO_CLE_1414 Parameter,CSV2Core.SessionSecurity.SessionSecurityTicket securityTicket = null){
            ...
            if (Parameter.ID != Guid.Empty)
            {
                ...
                {
                    account_id = cls_Get_Doctor_AccountID_for_DoctorID.Invoke(Connection, Transaction, new P_DO_GDAIDfDID_1549() { DoctorID = Parameter.ID }, securityTicket).Result.accountID;
                }

                if (Parameter.Type.ToLower().Equals("practice"))
                {
                    account_id = cls_Get_Practice_AccountID_for_PracticeID.Invoke(Connection, Transaction, new P_DO_GPAIDfPID_1522() { PracticeID = Parameter.ID }, securityTicket).Result.accountID;
                }
		...
		protected static FR_DO_CLE_1414 Invoke(DbConnection Connection, DbTransaction Transaction, string ConnectionString,P_DO_CLE_1414 Parameter,CSV2Core.SessionSecurity.SessionSecurityTicket securityTicket = null)
		{
			...
			catch (Exception ex)
			{
				...
				throw new Exception("Exception occured in method cls_Check_Login_Email",ex);
			}
...
[OperationContract]
FR_DO_CLE_1414 cls_Check_Login_Email(,P_DO_CLE_1414 Parameter, string sessionToken = null)
{
	try
	{
		var securityTicket = Verify(sessionToken);
		FR_DO_CLE_1414 invocationResult = cls_Check_Login_Email.Invoke(connectionString,P_DO_CLE_1414 Parameter,securityTicket);
...
var parameter = new DataImporter.DBMethods.Doctor.Atomic.Retrieval.P_DO_CLE_1414();
