...
(function () {
    ...
    define(['angularAMD'], function (angularAMD) {
            ...
            return {
                ...
                controller: ["$scope", "$attrs", "$element", function ($scope, $attrs, $element) {
                    ...
                    tagsInputConfig.load('tagsInput', $scope, $attrs, {
                        ...
                        additionalSearchProperty: [String, 'asp'],
                        keyProperty: [String, ''],
        ...
        tagsInput.directive('autoComplete', ["$document", "$timeout", "$sce", "$q", "tagsInputConfig", "tiUtil", function ($document, $timeout, $sce, $q, tagsInputConfig, tiUtil) {
            function SuggestionList(loadFn, options, events, parentInputName, suggestionListId) {
                ...
                function filterByQuery(items) {
                    var len = self.query.length;
                    var query = self.query.toLowerCase();
                    var filteredItems = [];
                    for (var i = 0; i < items.length; i++) {
                        var additionalSearchProperty = items[i][options.additionalSearchProperty || options.tagsInput.additionalSearchProperty];
                        var itemDisplayProperty = items[i][options.displayProperty || options.tagsInput.displayProperty];
                        var itemDisplayPropertyArray = itemDisplayProperty.split(' ');
                        var found = false;
                        for (var j = 0; j < itemDisplayPropertyArray.length; j++) {
                            if (itemDisplayPropertyArray[j].substring(0, len).toLowerCase() === query) {
                                found = true;
                                break;
                            };
                        };
                        ...
                        if (!found) {
                            found = itemDisplayProperty.substring(0, len).toLowerCase() === query;
                            if (!found && additionalSearchProperty !== undefined && additionalSearchProperty !== null) {
                                found = additionalSearchProperty.substring(0, len).toLowerCase() === query;
                            };
                        };
                ...
                self.show = function () {
                    angular.element('#' + suggestionListId)[0].style.left = angular.element('#' + parentInputName)[0].offsetLeft + 'px';
            ...
            return {
                restrict: 'E',
                require: '^tagsInput',
                scope: { source: '&', parentInputName: '=', suggestionListId: '=' },
                ...
                controller: ["$scope", "$element", "$attrs", function ($scope, $element, $attrs) {
                    ...
                    tagsInputConfig.load('autoComplete', $scope, $attrs, {
                        ...
                        additionalSearchProperty: [String, '']
                    });

                    $scope.suggestionList = new SuggestionList($scope.source, $scope.options, $scope.events, $scope.parentInputName, $scope.suggestionListId);
